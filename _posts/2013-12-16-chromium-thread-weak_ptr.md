...
weakptr的实现最底层的东西就是scoped_refptr。目标文件/src/base/memory/weak_ptr.h。WeakPtr主要是有如下的几个类构成：  
...
WeakFactory负责产生一个WeakPtr指针，通过WeakReferenceOwer产生一个WeakPtr<T>的指针。至于如何产生一个WeakPtr<T>指针请看WeakReferenceOwer中的GetRef()的实现。
对于如何使用的问题，请看下面：  
